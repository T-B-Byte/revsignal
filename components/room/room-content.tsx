...
type Tab = "products" | "quote" | "data-test" | "dpa" | "downloads" | string;
...
const ALL_DEMOS: Record<string, { label: string; url: string }> = {
  ...
  tal_matching: { label: "TAL Matching", url: "https://atlasiq.pharosiq.com/embed/pricing/raw-data" },
};
...
export function RoomContent({ room, products, slug, password, logId }: RoomContentProps) {
  ...
  return (
    <div className={`mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 min-h-screen transition-colors ${t("bg-slate-900 text-zinc-100", "bg-zinc-50 text-zinc-900")}`}>
      ...
      {activeTab === "downloads" && (
        <Downloads theme={theme} accentColor={accentColor} />
      )}
