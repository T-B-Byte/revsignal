...
/**
 * POST /api/ingest/call-transcripts
 *
 * Daily job: Pull Teams call transcripts, then process all unprocessed
...
export async function POST(request: NextRequest) {
  ...
  for (const token of tokens) {
    try {
      // Pull transcripts from yesterday
      ...
      console.error(
        `[ingest/call-transcripts] Failed for user ${token.user_id}:`,
